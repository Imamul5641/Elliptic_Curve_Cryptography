...
def addpoints(a,d,p,p1,p2):
  ...
  try:
    ...
    if p1[0] == p2[0] and p1[1] == p1[1]:
      # Doubling case
      l = (3 * p1[0] * p1[0] + a) * mod_inverse(2 * p1[1], p) % p
    else:
      # Addition case
      l = (p2[1] - p1[1]) * mod_inverse(p2[0] - p1[0], p) % p
...
def point_order(a, b, p, xp, yp):
    ...
    while True:
        try:
            xr, yr = addpoints(a, b, p, [xq, yq], [xq, yq])  # First doubling
            order +=1

            while (xr != xp or yr != yp):
                xq, yq = xr, yr
                xr, yr = addpoints(a, b, p, [xp, yp], [xq, yq])
                order += 1
